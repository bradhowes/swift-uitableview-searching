...
final class ArrayTransitionsTests: XCTestCase {
  ...
  func testEmptyFrom() throws {
    ...
    let to = [1, 2, 3]
    let changes = ArrayTransitions.changes(from: from, to: to)
    XCTAssertEqual(changes.added, [0, 1, 2])
  ...
  func testEmptyTo() throws {
    ...
    XCTAssertEqual(changes.deleted, [0, 1, 2])
  }
  ...
  func testLargeCollectionSizeDeletion() throws {
    ...
    let diff = ArrayTransitions.changes(from: from, to: to)
    XCTAssertEqual(diff.added.count, 0)
    XCTAssertEqual(diff.deleted.count, 99995)
  }
  ...
  func testLargeCollectionSizeInsertion() throws {
    ...
    let diff = ArrayTransitions.changes(from: from, to: to)
    XCTAssertEqual(diff.added.count, 99995)
